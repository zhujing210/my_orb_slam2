 ...
 */
cv::Mat Initializer::ComputeH21(const vector<cv::Point2f> &vP1,     //归一化后的点, in reference frame
...
{   
    // 说明：在已值 n 维观测数据误差服从N(0，sigma）的高斯分布时
    ...
    for(int i=0; i<N; i++)
    {
        ...
        const cv::KeyPoint &kp20 = mvKeys2[mvMatches12[i].second];
...
float Initializer::CheckFundamental(const cv::Mat &F21, vector<bool> &vbMatchesInliers, float sigma)
{
    ...
    //      所以，e =  (a * p2.x + b * p2.y + c) /  sqrt(a * a + b * b)ki
...
/**
给定投影矩阵P1, P2 和 图像上的匹配特征点kp1, kp2，从而计算三维点坐标
* @brief
*
...
{
    // 原理
    // Triangularization: 已知匹配特征点对 {x, x'} 和 各自相机矩阵 {P P'} ，估计三维点X
    // x' =  P'X, x = PX
    // 它们都属于 x = aPX 模型
    //                            |X|
 ...
 * @param[in] vMatchesInliers                     特征点对内点的标记
 * @param[in] K                                   相机内参矩阵
 * @param[in & out] vbGood                        标记成功三角化点
 * @param[in & out] Parallax                      计算出来的比较大的视差角（注意不是最大）
 */
int Initializer::CheckRT(const cv::Mat &R, const cv::Mat &t, const vector<cv::KeyPoint> &vKeys1, const vector<cv::KeyPoint> &vKeys2,
                       const vector<Match> &vMatches12, vector<bool> &vbMatchesInliers,
                       const cv::Mat &K, vector<cv::Point3f> &vP3D, float th2, vector<bool> &vbGood, float &parallax)
{
    // 对给出的特征点以及R、t，通过三角化检查解的有效性，也称为 cheirality check
    ...
    // step1 计算相机投影矩阵
    // 投影矩阵P是一个 3×4 矩阵，可以将空间中的一个点投影到平面上，获取其平面坐标，这里均指齐次坐标
    // 以第一个相机的光心作为世界坐标系，定义相机的投影矩阵 P1=K*[I|0]
    cv::Mat P1(3,4,CV_32F,cv::Scalar(0));
    K.copyTo(P1.rowRange(0,3).colRange(0,3));  // K*I = K
    ...
    for(size_t i=0, iend=vMatches12.size();i<iend;i++)
    {
        ...
        // step2 获取匹配的特征点对，调用 Tranulate() 函数进行三角化，得到三角测量之后的3D点坐标
        // kp1和 kp2 是匹配好的有效特征点
        ...
        // Check parallax
        // step3: 第一关：检查三角化的三维点坐标是否合法 (非无穷值)
        ...
        // check parallax
        // step4 第二关： 通过三维点深度值正负、两相机光心视差角大小来检查是否合法
        ...
        // 将空间点 p3dC1 变换到第2个相机坐标系下变为 p3dC2
        cv::Mat p3dC2 = R*p3dC1+t;

        if(p3dC2.at<float>(2)<=0 && cosParallax<0.99998)
            continue;

        // step 5 第三关： 计算空间点在参考帧和当前帧上的重投影误差，若大于阈值则舍弃
        // Check reprojection error in first image
        // 因为参考帧下的相机坐标系和世界坐标系重合，因此这里就直接进行投影就可以了
        float im1x, im1y;
        ...
        // 若运行到这里，说明当前遍历的这个特征点相对靠谱，经过了重重检验，是一个合格的点
    ...
    // step7 得到3D点中较大的视差角，并转换为角度制表示
    if(nGood>0)
    {
        // 从小到大排序，注意 vCosParallax 值越大，视差越小
        sort(vCosParallax.begin(),vCosParallax.end());

        // 排序后作者并没有取最小的视差角，而是取一个较小的视差角
        // 作者做法： 若经过检验后的有效3D点小于50个，那么就取那个最小的视差角(Cos值最大)
        // 若大于50个，就取排名第50个的较小的视差角即可，为了避免3D点太多时出校太小的视差角
